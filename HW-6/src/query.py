...
def div(col):
    if hasattr(col, "isSym") and col.isSym:
        ...
        if hasattr(col, 'col'):
            return (per(has(col.col),.9) - per(has(col.col), .1)) / 2.58
        else:
...
def norm(num, n):
    if hasattr(num, 'lo'):
        return n if n == "?" else (float(n) - num.lo) / (num.hi - num.lo + 1 / float("inf"))
    if hasattr(num.col, 'lo'):
        return n if n == "?" else (float(n) - num.col.lo) / (num.col.hi - num.col.lo + 1 / float("inf"))

    
    else:
        return n if n == "?" else float(n)
...
def dist(data, t1, t2, cols=None, d=None, dist1=None):
    ...
    for col in cols:
        d += dist1(col, t1[col.col.at], t2[col.col.at]) ** 2
